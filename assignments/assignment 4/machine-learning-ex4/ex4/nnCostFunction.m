...
for i=1:m,
  for k=1:num_labels,
    ...
    J = J - out * log(h(i, k)) - (1 - out) * log(1 - h(i, k));
  endfor
endfor
...
reg1_grad(:, 2:end) = (lambda / m) * Theta1(:, 2:end);

reg2_grad = zeros(size(Theta2_grad));
reg2_grad(:, 2:end) = (lambda / m) * Theta2(:, 2:end);

Theta1_grad = (1/m) * Theta1_grad + reg1_grad;
Theta2_grad = (1/m) * Theta2_grad + reg2_grad;
